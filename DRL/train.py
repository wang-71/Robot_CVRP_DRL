...
def train(cfg, log_path = None):
	...
	def rein_loss(model, inputs):
		...
		return ((L - L.mean()) * ll).mean(), L.mean()
	...
	for epoch in range(cfg.epochs):
		...
		for t, inputs in enumerate(dataloader):
			...
			loss, L_mean = rein_loss(model, inputs)
		...
		torch.save(model.state_dict(), '%s%s_epoch%s_' % (cfg.weight_dir, cfg.task, epoch) + cfg.dump_date + '.pt')
