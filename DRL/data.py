...
def generate_data(device, n_samples=10, n_customer=20, seed=None):
	...
	points_generated = ([(-4.8 + 0.6 * i, -1.5 + 3 * j) for i in range(17) for j in range(1)] +
						[(-4.8 + 0.6 * i, 1.5 + 3 * j) for i in range(17) for j in range(1)] +
						[(-4.8 + 0.6 * i, -0.5 + 1 * j) for i in range(17) for j in range(1)] +
						[(-4.2 + 0.6 * i, -0.5 + 1 * j) for i in range(15) for j in range(1)] +
						[(-4.8 + 0.6 * i, 0.5 + 1 * j) for i in range(17) for j in range(1)] +
						[(-4.2 + 0.6 * i, 0.5 + 1 * j) for i in range(15) for j in range(1)])

	capacity =  [0.2] * 34 + [0.1] * 64


	return (  # torch.rand((n_samples, 2), device = device),
		torch.tensor([(-8.8, 0) for _ in range(n_samples)], device=device),
		# torch.rand((n_samples, n_customer, 2), device = device),
		torch.tensor([points_generated for _ in range(n_samples)], device=device),
		# torch.randint(size = (n_samples, n_customer), low = 1, high = 10, device = device) / 20)
		torch.tensor([capacity for _ in range(n_samples)], device=device))
